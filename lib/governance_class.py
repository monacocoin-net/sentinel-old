...
class GovernanceClass(object):
    ...
    def vote(self, monacoCoind, signal, outcome):
        return self.go.vote(monacoCoind, signal, outcome)

    # pass thru to GovernanceObject#voted_on
    def voted_on(self, **kwargs):
        return self.go.voted_on(**kwargs)

    def vote_validity(self, monacoCoind):
        if self.is_valid():
            printdbg("Voting valid! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(monacoCoind, models.VoteSignals.valid, models.VoteOutcomes.yes)
        else:
            printdbg("Voting INVALID! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(monacoCoind, models.VoteSignals.valid, models.VoteOutcomes.no)
    ...
    def list(self):
        ...
        # return a dict similar to monacoCoind "gobject list" output
    ...
    def monacoCoind_serialise(self):
        import monacoCoinlib
        return monacoCoinlib.SHIM_serialise_for_monacoCoind(self.serialise())
