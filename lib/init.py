...
def has_monacoCoin_conf():
    import config
    import io

    valid_monacoCoin_conf = False

    # ensure monacoCoin_conf exists & readable
    #
    # if not, print a message stating that monacoCoin Core must be installed and
    # configured, including JSONRPC access in monacoCoin.conf
    try:
        f = io.open(config.monacoCoin_conf)
        valid_monacoCoin_conf = True
    except IOError as e:
        print(e)

    return valid_monacoCoin_conf
...
def main():
    ...
    if not has_monacoCoin_conf():
        print("monacoCoinCore must be installed and configured, including JSONRPC access in monacoCoin.conf")
        sys.exit(1)
