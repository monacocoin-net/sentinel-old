...
class monacoCoinDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some monacoCoind methods
    ...
    @classmethod
    def from_monacoCoin_conf(self, monacoCoin_dot_conf):
        from monacoCoin_config import monacoCoinConfig
        config_text = monacoCoinConfig.slurp_config_file(monacoCoin_dot_conf)
        creds = monacoCoinConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = monacoCoinlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = monacoCoinlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
