...
def is_valid_monacoCoin_address(address, network='mainnet'):
    ...
    monacoCoin_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid monacoCoin address
        return False

    if (address_version != monacoCoin_version):
        return False
...
# shims 'til we can fix the monacoCoind side
def SHIM_serialise_for_monacoCoind(sentinel_hex):
    from models import monacoCoinD_GOVOBJ_TYPES
    # unpack
    obj = deserialise(sentinel_hex)

    # shim for monacoCoind
    govtype = obj[0]

    # add 'type' attribute
    obj[1]['type'] = monacoCoinD_GOVOBJ_TYPES[govtype]

    # superblock => "trigger" in monacoCoind
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # monacoCoind expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    monacoCoind_hex = serialise(obj)
    return monacoCoind_hex


# shims 'til we can fix the monacoCoind side
def SHIM_deserialise_from_monacoCoind(monacoCoind_hex):
    from models import monacoCoinD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(monacoCoind_hex)

    # shim from monacoCoind
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('monacoCoin.conf')
