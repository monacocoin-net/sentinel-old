...
class GovernanceObject(BaseModel):
    ...
    def vote(self, monacoCoind, signal, outcome):
        import monacoCoinlib
        ...
        output = monacoCoind.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = monacoCoinlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(monacoCoind, signal)

    def sync_network_vote(self, monacoCoind, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = monacoCoind.get_my_gobject_votes(self.object_hash)
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    govobj_type = monacoCoinD_GOVOBJ_TYPES['watchdog']
    only_masternode_can_submit = True

    @classmethod
    def active(self, monacoCoind):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - monacoCoind.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, monacoCoind):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - monacoCoind.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, monacoCoind):
        now = int(time.time())
        return (self.created_at < (now - monacoCoind.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, monacoCoind):
        if self.is_expired(monacoCoind):
            return False

        return True

    def is_deletable(self, monacoCoind):
        if self.is_expired(monacoCoind):
            return True
