...
sentinel_cfg = monacoCoinConfig.tokenize(sentinel_config_file)


def get_monacoCoin_conf():
    home = os.environ.get('HOME')

    monacoCoin_conf = os.path.join(home, ".monacoCoinCore/monacoCoin.conf")
    if sys.platform == 'darwin':
        monacoCoin_conf = os.path.join(home, "Library/Application Support/monacoCoinCore/monacoCoin.conf")

    monacoCoin_conf = sentinel_cfg.get('monacoCoin_conf', monacoCoin_conf)

    return monacoCoin_conf
...
monacoCoin_conf = get_monacoCoin_conf()
network = get_network()
db = get_db_conn()
