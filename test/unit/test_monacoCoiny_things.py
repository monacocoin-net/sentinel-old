...
def test_valid_monacoCoin_address():
    from monacoCoinlib import is_valid_monacoCoin_address

    main = valid_monacoCoin_address()
    test = valid_monacoCoin_address('testnet')

    assert is_valid_monacoCoin_address(main) is True
    assert is_valid_monacoCoin_address(main, 'mainnet') is True
    assert is_valid_monacoCoin_address(main, 'testnet') is False

    assert is_valid_monacoCoin_address(test) is False
    assert is_valid_monacoCoin_address(test, 'mainnet') is False
    assert is_valid_monacoCoin_address(test, 'testnet') is True


def test_invalid_monacoCoin_address():
    from monacoCoinlib import is_valid_monacoCoin_address

    main = invalid_monacoCoin_address()
    test = invalid_monacoCoin_address('testnet')

    assert is_valid_monacoCoin_address(main) is False
    assert is_valid_monacoCoin_address(main, 'mainnet') is False
    assert is_valid_monacoCoin_address(main, 'testnet') is False

    assert is_valid_monacoCoin_address(test) is False
    assert is_valid_monacoCoin_address(test, 'mainnet') is False
    assert is_valid_monacoCoin_address(test, 'testnet') is False
...
def test_deterministic_masternode_elections(current_block_hash, mn_list):
    from monacoCoinlib import elect_mn
...
def test_parse_masternode_status_vin():
    from monacoCoinlib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % monacoCoinlib.hashit(sb_data_hex)
